...
export class FlightClientEntryPlugin {
      ...
      const recordModule = (modId, mod) => {
        ...
        const modPath =
          ...
          ((_mod_resourceResolveData = mod.resourceResolveData) == null
            ? void 0
            : _mod_resourceResolveData.path);
        const modQuery =
          ((_mod_resourceResolveData1 = mod.resourceResolveData) == null
            ? void 0
            : _mod_resourceResolveData1.query) || "";
      ...
      for (const clientEntryToInject of clientEntriesToInject) {
        const injected = this.injectClientEntryAndSSRModules({
          ...
          clientImports: {
            ...
            ...(dedupedCSSImports[clientEntryToInject.absolutePagePath] || []).reduce(
              (res, curr) => {
                res[curr] = new Set();
                return res;
              },
  ...
  collectComponentInfoFromServerEntryDependency({
    ...
    const filterClientComponents = (mod, importedIdentifiers) => {
      ...
      getModuleReferencesInOrder(mod, compilation.moduleGraph).forEach((connection) => {
        ...
        if (
          (_connection_dependency = connection.dependency) == null
            ? void 0
            : _connection_dependency.ids
        ) {
  ...
  injectClientEntryAndSSRModules({
    ...
    return [
      ...
      this.addEntry(
        compilation, // Reuse compilation context.
        compiler.context,
  ...
  async createActionAssets(compilation, assets) {
    ...
    assets[`${this.assetPrefix}${SERVER_REFERENCE_MANIFEST}.js`] = new sources.RawSource(
...
function addClientImport(
  ...
  const clientEntryType =
    (_getModuleBuildInfo_rsc = getModuleBuildInfo(mod).rsc) == null
      ? void 0
      : _getModuleBuildInfo_rsc.clientEntryType;
...
function getModuleResource(mod) {
  ...
  if (
    (_mod_matchResource = mod.matchResource) == null
      ? void 0
      : _mod_matchResource.startsWith(BARREL_OPTIMIZATION_PREFIX)
  ) {
