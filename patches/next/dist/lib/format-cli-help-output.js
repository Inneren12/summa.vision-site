...
const formatCliHelpOutput = (cmd, helper) => {
  ...
  // Usage
  let output = [`${(0, _picocolors.bold)("Usage:")} ${helper.commandUsage(cmd)}`, ""];
  // Description
  ...
  if (optionList.length > 0) {
    output = output.concat([`${(0, _picocolors.bold)("Options:")}`, formatList(optionList), ""]);
  }
  // Commands
  ...
  if (commandList.length > 0) {
    output = output.concat([`${(0, _picocolors.bold)("Commands:")}`, formatList(commandList), ""]);
  }
  return output.join("\n");
};
