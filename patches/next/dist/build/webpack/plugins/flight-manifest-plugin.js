...
function entryNameToGroupName(entryName) {
  let groupName = entryName
    .slice(0, entryName.lastIndexOf("/")) // Remove slots
...
class ClientReferenceManifestPlugin {
  ...
  apply(compiler) {
    compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation, { normalModuleFactory }) => {
      compilation.dependencyFactories.set(
        _webpack.webpack.dependencies.ModuleDependency,
        normalModuleFactory,
      );
      compilation.dependencyTemplates.set(
        _webpack.webpack.dependencies.ModuleDependency,
        new _webpack.webpack.dependencies.NullDependency.Template(),
      );
  ...
  createAsset(assets, compilation, context) {
    ...
    (_compilation_entrypoints_get = compilation.entrypoints.get(
      _constants.CLIENT_STATIC_FILES_RUNTIME_MAIN_APP,
    )) == null
      ? void 0
      : _compilation_entrypoints_get.getFiles().forEach((file) => {
    ...
    for (let [entryName, entrypoint] of compilation.entrypoints) {
      if (
        entryName === _constants.CLIENT_STATIC_FILES_RUNTIME_MAIN_APP ||
        entryName === _constants.APP_CLIENT_INTERNALS
      ) {
        entryName = "";
      ...
      const recordModule = (modId, mod) => {
        ...
        // module ID strategy.
        let ssrNamedModuleId = (0, _path.relative)(
          context,
          ((_mod_resourceResolveData = mod.resourceResolveData) == null
            ? void 0
            : _mod_resourceResolveData.path) || resource,
        );
        ...
        const esmResource = /[\\/]next[\\/]dist[\\/]/.test(resource)
          ? resource.replace(
              /[\\/]next[\\/]dist[\\/]/,
              "/next/dist/esm/".replace(/\//g, _path.default.sep),
            )
          : null;
        ...
        if (
          (_mod_matchResource = mod.matchResource) == null
            ? void 0
            : _mod_matchResource.startsWith(_constants.BARREL_OPTIMIZATION_PREFIX)
        ) {
          ssrNamedModuleId = (0, _utils.formatBarrelOptimizedResource)(
            ssrNamedModuleId,
            mod.matchResource,
          );
      ...
      function recordChunkGroup(chunkGroup) {
        ...
        chunkGroup.chunks.forEach((chunk) => {
          ...
          for (const mod of entryMods) {
            ...
            const connections = (0, _utils.getModuleReferencesInOrder)(
              mod,
            ...
            for (const connection of connections) {
              ...
              if (modId !== null) {
                ...
                if (
                  ((_connection_module = connection.module) == null
                    ? void 0
                    : _connection_module.constructor.name) === "ConcatenatedModule"
                ) {
    ...
    for (const pageName of manifestEntryFiles) {
      ...
      const pageBundlePath = (0, _normalizepagepath.normalizePagePath)(
        pagePath.slice("app".length),
      );
      assets["server/app" + pageBundlePath + "_" + _constants.CLIENT_REFERENCE_MANIFEST + ".js"] =
        new _webpack.sources.RawSource(
          `globalThis.__RSC_MANIFEST=(globalThis.__RSC_MANIFEST||{});globalThis.__RSC_MANIFEST[${JSON.stringify(pagePath.slice("app".length))}]=${json}`,
        );
    }
