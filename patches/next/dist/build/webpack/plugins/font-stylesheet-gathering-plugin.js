...
const _cssnanosimple = /*#__PURE__*/ _interop_require_default(
  require("next/dist/compiled/cssnano-simple"),
);
        ...
        factory.hooks.parser.for("javascript/" + type).tap(this.constructor.name, (parser) => {
           ...
           */ parser.hooks.evaluate.for("Identifier").tap(this.constructor.name, (node) => {
            ...
            if (
              parser == null
                ? void 0
                : (_parser_state = parser.state) == null
                  ? void 0
                  : (_parser_state_module = _parser_state.module) == null
                    ? void 0
                    : _parser_state_module.resource.includes("node_modules")
            ) {
              return;
            }
            let result;
          ...
          const jsxNodeHandler = (node) => {
            ...
            if (
              ...
              !props.href ||
              !_constants.OPTIMIZED_FONT_PROVIDERS.some(({ url }) => props.href.startsWith(url))
            ) {
              return false;
            }
            ...
            const buildInfo =
              parser == null
                ...
                : (_parser_state = parser.state) == null
                  ...
                  : (_parser_state_module = _parser_state.module) == null
                    ? void 0
                    : _parser_state_module.buildInfo;
            if (buildInfo) {
              buildInfo.valueDependencies.set(
                _constants.AUTOMATIC_FONT_OPTIMIZATION_MANIFEST,
                this.gatheredStylesheets,
              );
            }
          };
  ...
  apply(compiler) {
        ...
        async (modules, modulesFinished) => {
          ...
          modules.forEach((module) => {
            ...
            const fontDependencies =
              module == null
                ? void 0
                : (_module_buildInfo = module.buildInfo) == null
                  ? void 0
                  : (_module_buildInfo_valueDependencies = _module_buildInfo.valueDependencies) ==
                      null
                    ? void 0
                    : _module_buildInfo_valueDependencies.get(
                        _constants.AUTOMATIC_FONT_OPTIMIZATION_MANIFEST,
                      );
            if (fontDependencies) {
          ...
          const fontDefinitionPromises = fontStylesheets.map((url) =>
            (0, _fontutils.getFontDefinitionFromNetwork)(url),
          );
          this.manifestContent = [];
          for (let promiseIndex in fontDefinitionPromises) {
            ...
            if (this.adjustFontFallbacks) {
              css += (0, _fontutils.getFontOverrideCss)(
          ...
          compilation.assets[_constants.AUTOMATIC_FONT_OPTIMIZATION_MANIFEST] =
            new _webpack.sources.RawSource(JSON.stringify(this.manifestContent, null, "  "));
          modulesFinished();
        },
      );
      cb();
    ...
    compiler.hooks.make.tap(this.constructor.name, (compilation) => {
      compilation.hooks.processAssets.tap(
        ...
        (assets) => {
          assets["../" + _constants.AUTOMATIC_FONT_OPTIMIZATION_MANIFEST] =
            new _webpack.sources.RawSource(JSON.stringify(this.manifestContent, null, "  "));
        },
      );
    });
