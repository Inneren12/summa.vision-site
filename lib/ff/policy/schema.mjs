...
const CanaryMemberSchema = z
  .object({
    userId: NonEmptyString.describe("User identifier to enroll in canary").optional(),
    ff_aid: NonEmptyString.describe("ff_aid cookie value to enroll in canary").optional(),
  })
  .strict()
  .superRefine((value, ctx) => {
    if (!value.userId && !value.ff_aid) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "canary cohort entry must include userId and/or ff_aid",
      });
    }
  })
  .transform((value) => ({
...
const CanarySchema = z
  .object({
    ttlHours: z
      .number({ invalid_type_error: "canary.ttlHours must be a number" })
      .int({ message: "canary.ttlHours must be an integer" })
      .min(1, { message: "canary.ttlHours must be within [1..168] hours" })
      .max(168, { message: "canary.ttlHours must be within [1..168] hours" }),
    cohort: z
      .array(CanaryMemberSchema, {
        invalid_type_error: "canary.cohort must be an array",
      })
      .nonempty({ message: "canary.cohort must contain at least one entry" }),
  })
  .strict();
